...
describe('jobs-service', () => {
  ...
  it('executes job by id', async () => {
    await expect(executeJobById('j1')).resolves.toBeUndefined();
  });

  it('sets job retries by id', async () => {
    await expect(setJobRetriesById('j1', { retries: 3 })).resolves.toBeUndefined();
  });
  ...
  it('sets job definition retries by id', async () => {
    await expect(setJobDefinitionRetriesById('jd1', { retries: 2 })).resolves.toBeUndefined();
  });
});
