...
vi.mock('@src/features/mission-control/processes-overview/hooks', async () => {
  ...
  return {
    ...actual,
    useProcessesData: () => ({
      defsQ: { data: [], isLoading: false, error: { status: 503, message: 'No engine' } },
      defItems: [],
      versions: [],
      currentKey: '',
...
describe('ProcessesOverviewPage error state', () => {
  it('renders engine access error when defs request fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No Active Engine')).toBeInTheDocument();
    });
  });
});
